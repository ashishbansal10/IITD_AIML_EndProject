...
from experiment import ExperimentConfig, ExecutionerConfig, ExperimentRunner, ResultStore
...
def _get_current_function_name():
    # frame = inspect.currentframe() # This gets the frame of the helper function itself
    # use f_back to get the caller's frame
...
# ==============================================================================
# smoke_test0 — Data pipeline only  (<2 min, no disk writes)
# ==============================================================================

def run_smoke_test0(loader_factory, device, num_workers=0):
    """
    Smoke test 0 — Data pipeline verification only. No model, no training.
    Verifies: pool counts, disjointness, loader shapes for batch + episodic.
    No disk writes.
    """
    print(f"\n{'='*55}")
    print(f"{_get_current_function_name()} — Data pipeline  |  device={device}")
    print(f"{'='*55}\n")
    ...
    print(f"{_get_current_function_name()} done. No disk writes.\n")
    return all_ok

# ==============================================================================
# smoke_test1 — Forward pass only, all 3 archs × 4 modes  (<30s, no disk)
# ==============================================================================

def run_smoke_test1(loader_factory, device):
    """
    ...
    """
    print(f"\n{'='*55}")
    print(f"{_get_current_function_name()} — Forward pass  |  device={device}")
    print(f"{'='*55}")
...
def run_smoke_test2(loader_factory, device, num_workers=0):
    ...
    """
    DIRS = ['smoke2_checkpoints', 'smoke2_results', 'smoke2_plots', 'smoke2_logs']

    print(f"\n{'='*55}")
    print(f"{_get_current_function_name()} — All 6 runs, PyTorch  |  device={device}")
    print(f"{'='*55}\n")
    ...
    print("\nCleaning up...")
    _cleanup(*DIRS)
    print(f"{_get_current_function_name()} done.\n")


# ==============================================================================
# smoke_test3 — CNN pipeline, pretrain + Optuna  (~5 min)
# ==============================================================================
...
def run_smoke_test3(loader_factory, device, num_workers=0):
    ...
    Cleans up all disk files on completion.
    """
    DIRS = ['smoke3_checkpoints', 'smoke3_results', 'smoke3_plots', 'smoke3_logs']

    print(f"\n{'='*55}")
    print(f"{_get_current_function_name()} — Optuna  |  device={device}")
    print(f"{'='*55}\n")
    ...
    exec_cfg = ExecutionerConfig(
        checkpoint_dir=DIRS[0], results_dir=DIRS[1], plots_dir=DIRS[2], logs_dir=DIRS[3], num_workers=num_workers,
    )

    runner  = ExperimentRunner(run_configs, exec_cfg, loader_factory, device)
    best_hps = runner.tune_all()
    print(f"\nBest hyperparameters from tuning: {best_hps}")
